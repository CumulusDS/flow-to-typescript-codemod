...
export class CumulusSteps {
  ...
  private addDefaultSteps() {
    ...
    this.addStep("Install TypeScript as a dev dependency", () => runCommands(["yarn add typescript --dev"]));
    this.addStep("Remove flow artifacts", () => runCommands(["rm -rf flow-typed", "rm -f .flowconfig"]));
    ...
    this.addStep("Remove babel old dependencies", () => runCommands(["yarn remove @babel/preset-flow babel-eslint"]));
    this.addStep("Add babel new dependencies", () => runCommands(["yarn add @babel/preset-typescript --dev"]));
    ...
    this.addStep("Update eslint to ^8.57.1", () => runCommands(["yarn add eslint@^8.57.1 --dev"]));
    this.addStep("Install eslint plugin and parser ", () =>
      runCommands(["yarn add @typescript-eslint/eslint-plugin @typescript-eslint/parser --dev"])
    );
    this.addStep("Update prettier to ^3.3.3", () => runCommands(["yarn add prettier@^3.3.3 --dev"]));
    this.addStep("Update eslint-config-prettier to ^8.10.0", () =>
      runCommands(["yarn add eslint-config-prettier@^8.10.0 --dev"])
    );
    this.addStep("Update eslint-plugin-prettier to ^5.2.1", () =>
      runCommands(["yarn add eslint-plugin-prettier@^5.2.1 --dev"])
    );
    this.addStep("Add eslint-plugin-import", () => runCommands(["yarn add eslint-plugin-import --dev"]));
    ...
    this.addStep("Remove @cumulusds/flow-aws-sdk", () => runCommands(["yarn @cumulusds/flow-aws-sdk"]));
    this.addStep("Remove flow-bin", () => runCommands(["yarn remove flow-bin"]));
    this.addStep("Remove flow-copy-source", () => runCommands(["yarn remove flow-copy-source"]));
    this.addStep("Remove flow-typed", () => runCommands(["yarn remove flow-typed"]));

    this.addStep("Add @types/aws-lambda", () => runCommands(["yarn add @types/aws-lambda --dev"]));
    this.addStep("Add @types/jest", () => runCommands(["yarn add @types/jest --dev"]));
    this.addStep("Add flowgen", () => runCommands(["yarn add flowgen --dev"]));
  }
